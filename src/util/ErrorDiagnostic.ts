...
${error.stack || 'No stack trace available'}
        ...
        const writes: Promise<void>[] = [fs.writeFile(path.join(outputDir, 'error.txt'), errorLog)]
        if (htmlContent) writes.push(fs.writeFile(path.join(outputDir, 'dump.html'), htmlContent))
        if (screenshotBuffer) writes.push(fs.writeFile(path.join(outputDir, 'screenshot.png'), screenshotBuffer))
