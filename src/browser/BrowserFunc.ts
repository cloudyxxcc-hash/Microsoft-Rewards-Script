...
export default class BrowserFunc {
    ...
    async getDashboardData(cookies?: Cookie[]): Promise<DashboardData> {
        ...
        if (activePage) {
            this.bot.logger.debug(this.bot.isMobile, 'GET-DASHBOARD-DATA', 'Fetching dashboard data via active browser page')
            try {
                const response = await activePage.request.get(URLs.rewards.userInfoApi, {
                    headers: {
                ...
                if (response.ok()) {
                    ...
                    if (data?.dashboard) {
                        await this.syncActiveCookies(activePage, 'GET-DASHBOARD-DATA-BROWSER')
                ...
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'GET-DASHBOARD-DATA',
                    `Browser-based dashboard request failed; falling back to http | message=${this.errorMessage(error)}`
                )
            }
        }
    ...
    private async getFlyoutDashboardData(
        ...
        const activePage = this.getActivePage()
        if (activePage) {
            this.bot.logger.debug(this.bot.isMobile, 'GET-DASHBOARD-DATA', 'Fetching flyout dashboard via active browser page')
            try {
                const response = await activePage.request.get(URLs.bing.rewardsFlyoutUserInfo, {
                    headers: {
                ...
                if (response.ok()) {
                    const data = await response.json().catch(() => null)
                    if (data && !data.isError && data.userInfo?.isRewardsUser) {
                        await this.syncActiveCookies(activePage, 'GET-FLYOUT-DASHBOARD-BROWSER')
                        const detection = detectFlyoutBotWarning(data as RewardsFlyoutData)
                        this.bot.logger.warn(
                            this.bot.isMobile,
                            'GET-DASHBOARD-DATA',
                        ...
                        return mapFlyoutToDashboard(data as RewardsFlyoutData)
                    }
                }
            } catch (error) {
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'GET-DASHBOARD-DATA',
                    `Browser-based flyout request failed; falling back to http | message=${this.errorMessage(error)}`
                )
            }
        }
    ...
    async getAppEarnablePoints(): Promise<AppEarnablePoints> {
        try {
            ...
            const eligibleActivities = (userData?.response?.promotions ?? []).filter(x => {
                const offerId = (x.attributes?.offerid ?? '').toLowerCase()
                const type = (x.attributes?.type ?? '').toLowerCase()
                return (
            ...
            for (const item of eligibleActivities) {
                ...
                if (attrs.type === 'msnreadearn' || (attrs.offerid ?? '').toLowerCase().includes('readarticle')) {
                    const pointMax = parseInt(attrs.pointmax ?? '30')
                    const pointProgress = parseInt(attrs.pointprogress ?? '0')
                    readToEarn = Math.max(0, pointMax - pointProgress)
                } else if (attrs.type === 'checkin' || (attrs.offerid ?? '').toLowerCase().includes('dailycheckin')) {
