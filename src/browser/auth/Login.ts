...
export class Login {
    ...
    private passwordSignInBlocked = false

    private readonly selectors = {
        primaryButton:
            'button[data-testid="primaryButton"], button#idSIButton9, input#idSIButton9, button[type="submit"], input[type="submit"], button#acceptButton',
        secondaryButton:
            'button[data-testid="secondaryButton"], button#idBtn_Back, input#idBtn_Back, button#declineButton',
        footerAction:
            '[data-testid="viewFooter"] [role="button"], #idA_PWD_SwitchToPassword, #signInAnotherWay, #otherWays, a#idA_PWD_ForgotPassword',
        signInTile:
            '[data-testid="tile"], .tile-container, div[data-test="account-tile"], div[role="button"][data-bind*="tile"], div.tile',
        ...
        emailVerificationInput: 'input#proof-confirmation-email-input, input#iProofEmail',
        accountLocked: '#serviceAbuseLandingTitle',
        errorAlert:
            'div#usernameError, div#passwordError, div#error, div#error_alert, div.error, .alert-error, div[role="alert"]#error, div[role="alert"].error',
        passwordEntry:
            '[data-testid="passwordEntry"], input[type="password"], input[name="passwd"], input#i0118',
        emailEntry:
            'input#usernameEntry, input[name="loginfmt"], input#i0116, input[type="email"]',
        kmsiVideo:
            '[data-testid="kmsiVideo"], #kmsiTitle, #kmsiDescription, input[name="kmsi"]',
        ...
        totpInput: 'input[name="otc"], input#idTxtBx_SAOTCC_OTC',
        totpInputOld: 'form[name="OneTimeCodeViewForm"]',
        identityBanner: '[data-testid="identityBanner"], #displayName, #userDisplayName',
        otpCodeEntry: '[data-testid="codeEntry"], input[name="otc"]',
        backButton: '#back-button, button#idBtn_Back, input#idBtn_Back',
        bingProfile: '#id_n',
    ...
    async login(page: Page, account: Account) {
        try {
            ...
            this.passwordSignInBlocked = false
            ...
            while (iteration < maxIterations) {
                ...
                if (state !== previousState && previousState !== 'UNKNOWN') {
                    ...
                    if (state !== 'UNKNOWN' && state !== 'CHROMEWEBDATA_ERROR') {
                        iteration = Math.max(1, iteration - 5)
                    }
                }
                ...
                if (state === previousState && state !== 'LOGGED_IN') {
                    ...
                    if (sameStateCount >= 4) {
                        this.bot.logger.warn(
                            this.bot.isMobile,
                            'LOGIN',
                            `Stuck in state "${state}" for 4 loops, attempting page recovery`
                        )
                        if (state === 'UNKNOWN') {
                            await page
                                .goto(URLs.rewards.userLogin, { waitUntil: 'domcontentloaded', timeout: 15000 })
                                .catch(() => {})
                        } else {
    ...
    private async detectCurrentState(page: Page): Promise<LoginState> {
        ...
        if (foundStates.includes('ERROR_ALERT')) {
            ...
            if (errorIsReal) {
                const errorText = await page.locator(this.selectors.errorAlert).first().innerText().catch(() => '')
                if (errorText.trim().length > 0) {
                    ...
                    this.bot.logger.debug(
                        this.bot.isMobile,
                        'DETECT-STATE',
                        'ERROR_ALERT element was found but had no text content; ignoring'
                    )
                }
            }
    ...
    private async handleState(state: LoginState, page: Page, account: Account): Promise<boolean> {
        ...
        switch (state) {
            ...
            case 'ERROR_ALERT': {
                ...
                if (
                    lowerMsg.includes('password sign-in') ||
                    lowerMsg.includes('try another method') ||
                    lowerMsg.includes("isn't available")
                ) {
                    this.passwordSignInBlocked = true
                    this.bot.logger.warn(
                        this.bot.isMobile,
                        'LOGIN',
                        'Password sign-in is blocked/disabled by Microsoft. Attempting fallback to alternative sign-in methods.'
                    )
                    if (await this.tryClick(page, this.selectors.backButton, 'Back button')) {
                        return true
                    }
                    if (await this.tryClick(page, this.selectors.footerAction, 'Alternative methods link')) {
                        return true
                    }
                }
            ...
            case 'SIGN_IN_METHOD_PICKER': {
                ...
                const accountOption = options.find(opt =>
                    opt.label.toLowerCase().includes(account.email.toLowerCase())
                )
                if (accountOption) {
                    this.bot.logger.info(
                        this.bot.isMobile,
                        'LOGIN',
                        `Account tile for ${account.email} found; selecting it`
                    )
                    this.passwordlessMethodSelected = false
                    if (await this.clickSignInMethodOption(page, accountOption)) {
                        await this.waitForIdle(page, 'after account tile selection')
                        return true
                    }
                }
                ...
                if (account.password && passwordOption && !this.passwordSignInBlocked) {
                ...
                if (account.totpSecret && totpOption) {
                    this.bot.logger.info(this.bot.isMobile, 'LOGIN', 'Selecting TOTP verification sign-in method')
                    this.passwordlessMethodSelected = false

                    if (!(await this.clickSignInMethodOption(page, totpOption))) {
                        this.bot.logger.warn(this.bot.isMobile, 'LOGIN', 'Could not select TOTP sign-in method')
                        return false
                    }

                    await this.waitForIdle(page, 'after TOTP method selection')
                    return true
                }
