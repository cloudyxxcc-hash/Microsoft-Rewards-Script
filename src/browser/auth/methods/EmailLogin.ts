...
export class EmailLogin {
    private submitButton =
        'button[type="submit"], input[type="submit"], button#idSIButton9, input#idSIButton9, button[data-testid="primaryButton"]'
    ...
    async enterEmail(page: Page, email: string): Promise<'ok' | 'error'> {
        try {
            const emailInputSelector =
                'input[name="loginfmt"], input[type="email"], input#i0116, input#usernameEntry, input[type="text"]'
            const emailField = await page
                .waitForSelector(emailInputSelector, { state: 'visible', timeout: 2000 })
                .catch(() => {})
            if (!emailField) {
            ...
            const prefilledEmail = await page
                .waitForSelector('#userDisplayName, #displayName', { state: 'visible', timeout: 1000 })
                .catch(() => {})
            if (!prefilledEmail) {
    ...
    async enterPassword(page: Page, password: string): Promise<'ok' | 'needs-2fa' | 'error'> {
        try {
            const passwordInputSelector =
                'input[type="password"], input[name="passwd"], input#i0118, [data-testid="passwordEntry"] input'
            const passwordField = await page
                .waitForSelector(passwordInputSelector, { state: 'visible', timeout: 2000 })
                .catch(() => {})
            if (!passwordField) {
